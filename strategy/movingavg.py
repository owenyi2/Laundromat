...
class Trader:
    ...
    def compute_starfruit_fair_value(self, midprice, slow_ema, fast_ema) -> int:
        intercept = 0
    ...
    def compute_starfruit_order(self, state: TradingState) -> list[Order]:
        ...
        # previous_ask_vol = self.traderData["STARFRUIT"]["previous_ask_vol"] 
        # self.traderData["STARFRUIT"]["previous_ask_vol"] = best_ask_vol 
        # previous_bid_vol = self.traderData["STARFRUIT"]["previous_bid_vol"] 
        # self.traderData["STARFRUIT"]["previous_bid_vol"] = best_bid_vol 

        position = state.position.get("STARFRUIT", 0)
        order_depth: OrderDepth = state.order_depths["STARFRUIT"]
        

        # midprice = (best_ask_pr*best_ask_vol + best_bid_pr*best_bid_vol) / (best_bid_vol + best_ask_vol) 
        # previous_midprice = (previous_ask*previous_ask_vol + previous_bid*previous_bid_vol) / (previous_bid_vol + previous_ask_vol)
        ...
        fair_value = self.compute_starfruit_fair_value(midprice, slow_ema, fast_ema)
        
        # print(f"fair,{fair_value}")
        # print(best_bid_vol + best_ask_vol)
    ...
    def parse_trader_data(self, state: TradingState):
        if state.traderData == '':
            ...
            self.traderData = {"STARFRUIT": {"previous_ask": 1e9, "previous_bid": -1e9, "previous_ask_vol": 1, "previous_bid_vol": 1, "slow_ema": None, "fast_ema": None, "KF_state": jsonpickle.encode((x, P))}}
        else:
